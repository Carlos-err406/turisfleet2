...
import { Cookie } from "$lib/server-utils";
...
export const POST: RequestHandler = async ({ request, cookies }) => {
    ...
    cookies.set(Cookie.TOKEN, token, { maxAge: 60 * 60 * 24, secure: true, path: '/', httpOnly: true, sameSite: true })
    cookies.set(Cookie.REFRESH, refreshToken, { secure: true, path: '/', httpOnly: true, sameSite: true })
